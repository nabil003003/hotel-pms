...
def assert_path_establishment_access(user: CurrentUser, establishment_id) -> None:
    """Variante path-param de require_establishment_access : ici
    l'établissement ciblé est dans l'URL (`/establishments/{id}/...`), pas
    dans le header X-Establishment-Id (celui-ci reste l'usage recommandé côté
    frontend pour le sélecteur de Riad, D2)."""
    if user.is_super_admin:
        return
